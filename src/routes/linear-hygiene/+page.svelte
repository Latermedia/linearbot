<script lang="ts">
  ...
  onMount(async () => {
    if (!browser) return;
    pageLoading.startLoading("/linear-hygiene");
    ...
    try {
      ...
      loading = false;
      pageLoading.stopLoading("/linear-hygiene");
    }
  });
  ...
  {:else if linearHygiene}
    ...
    <div class="py-8 border-b border-black-200 dark:border-white/10">
      ...
      <div class="flex items-baseline justify-center gap-4 mb-3">
        <span
          class="text-8xl lg:text-9xl font-bold tracking-tight drop-shadow-sm dark:drop-shadow-none {getStatusTextColor(
