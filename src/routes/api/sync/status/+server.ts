...
export const GET: RequestHandler = async () => {
	...
	if (partialSyncState) {
		const completedProjects = partialSyncState.projectSyncs.filter(p => p.status === 'complete').length;
		const totalProjects = partialSyncState.projectSyncs.length;
		if (totalProjects > 0) {
			partialSyncProgress = { completed: completedProjects, total: totalProjects };
		}
	}
