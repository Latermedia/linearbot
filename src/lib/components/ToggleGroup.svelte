<script lang="ts" module>
  ...
  export function toggleVariants(variant: Variant, size: Size = "default") {
    ...
    const variants = {
      default:
        "hover:bg-white/10 data-[state=on]:bg-white/10 data-[state=on]:text-white text-neutral-400 bg-transparent rounded",
      outline:
        "hover:bg-white/5 data-[state=on]:bg-transparent data-[state=on]:text-white text-neutral-400 bg-transparent rounded-md",
    };
