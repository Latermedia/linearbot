...
<div
  class="fixed z-50 px-3 py-2 bg-black-900 border border-white/10 rounded-lg shadow-xl min-w-[200px] max-w-[280px]"
  ...
  <div class="space-y-1 max-h-[200px] overflow-y-auto">
    {#each engineers as engineer (engineer.assignee_id)}
      ...
      >
        ...
        {:else}
          <div
            class="w-5 h-5 rounded-full bg-black-700 flex items-center justify-center text-[10px] text-black-400"
          >
            {engineer.assignee_name.charAt(0).toUpperCase()}
          </div>
        {/if}
        <span class="text-sm text-black-200 truncate"
